...
case class MetadataBlockBuilder(displayName: String) {
  ...
  def withSingleValueField(fieldId: FieldId, value: String): MetadataBlockBuilder = {
    fields.append(PrimitiveSingleValueField(typeName = fieldId, multiple = false, typeClass = TYPE_CLASS_PRIMITIVE, value = value))
    this
  }

  def withMultiValueField(fieldId: FieldId, values: List[String]): MetadataBlockBuilder = {
    fields.append(PrimitiveMultipleValueField(typeName = fieldId, multiple = true, typeClass = TYPE_CLASS_PRIMITIVE, value = values))
    this
  }

  def withControlledSingleValueField(fieldId: FieldId, value: String): MetadataBlockBuilder = {
    fields.append(PrimitiveSingleValueField(typeName = fieldId, multiple = false, typeClass = TYPE_CLASS_CONTROLLED_VOCABULARY, value = value))
    this
  }

  def withControlledMultiValueField(fieldId: FieldId, values: List[String]): MetadataBlockBuilder = {
    fields.append(PrimitiveMultipleValueField(typeName = fieldId, multiple = true, typeClass = TYPE_CLASS_CONTROLLED_VOCABULARY, value = values))
    this
  }
