...
object UpdateDatasetMetadata extends App with DebugEnhancedLogging with BaseApp {
   ...
   */
  private val newMetadataBlocks: MetadataBlocks = Map(
    "citation" -> MetadataBlock(
      ...
      fields = List(
        ...
        CompoundField("dsDescription", List(
          toFieldMap(
            PrimitiveSingleValueField("dsDescriptionValue", "NEW DESCRIPTION")
          )
        )),
  ...
  val result = for {
    response <- server.dataset(persistentId).updateMetadata(newMetadataBlocks)
