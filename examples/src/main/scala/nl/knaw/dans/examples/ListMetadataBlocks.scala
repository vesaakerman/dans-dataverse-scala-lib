...
object ListMetadataBlocks extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val persistentId = args(0)

  val result = for {
    response <- server.dataset(persistentId).listMetadataBlocks()
    ...
    _ = logger.info(s"Display names: ${ blocks.values.map(_.displayName).mkString(", ") }")
  } yield ()
