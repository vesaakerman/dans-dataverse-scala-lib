...
object RestrictFile extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val databaseId = args(0).toInt
  private val restrict = args(1).toBoolean

  val result = for {
    response <- server.file(databaseId).restrict(restrict)
