...
object PublishDataset extends App with DebugEnhancedLogging with BaseApp {
  private implicit val jsonFormats: Formats = DefaultFormats
  private val persistentId = args(0)

  val result = for {
    response <- server.dataset(persistentId).publish(UpdateType.major)
    ...
    publicationResult <- response.data
    _ = logger.info(s"Publisher: ${ publicationResult.publisher }")
  } yield ()
